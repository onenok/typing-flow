...
export default function PracticePage() {
  ...
  useEffect(() => {
    if (paramLevel) {
      const foundLevel = PracticeLEVELS.find(l => l.id === paramLevel);
      if (foundLevel) {
  ...
  const handleNextLevel = useCallback(() => {
    ...
    const nextIndex = PracticeLEVELS.findIndex(l => l.id === selectedLevel.id) + 1;
    if (nextIndex < PracticeLEVELS.length) {
      setSelectedLevel(PracticeLEVELS[nextIndex]);
    } else {
      setSelectedLevel(null);
    }
  }, [selectedLevel]);
          ...
          <div className="max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="text-right">
                <div className="text-xs font-bold text-blue-500 uppercase tracking-widest">正在練習</div>
                <div className="text-xl font-black text-gray-800">{selectedLevel.title}</div>
              </div>
            </div>

            <TypingModule
              key={selectedLevel.id} // Ensure TypingModule resets when level changes
